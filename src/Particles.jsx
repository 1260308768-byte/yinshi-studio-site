...
const defaultColors = ['#ffffff', '#dffaf7', '#7aefe1']

function hexToRgb(hex) {
  const normalized = hex.replace(/^#/, '').replace(/^(.)(.)(.)$/, '$1$1$2$2$3$3')
...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    if (uAlphaParticles < 0.5) {
      ...
      float circle = smoothstep(0.5, 0.38, d) * 0.72;
...
function Particles({
  ...
  useEffect(() => {
    ...
    const activeSpeed = reduceMotion ? Math.max(speed * 0.6, 0.045) : speed

    const renderer = new Renderer({
      dpr: Math.min(pixelRatio, window.devicePixelRatio || 1),
      depth: false,
      alpha: true,
    })
    ...
    const update = (time) => {
      ...
      if (!disableRotation) {
        ...
        particles.rotation.z += 0.006 * activeSpeed
      }
